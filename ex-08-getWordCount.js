...
var getWordCount = function(someString){
  var countObj = {}
  var wordsWithoutPunct = ''
  for(var k = 0; k < someString.length; k++){
    if(removePunctuation(someString[k]) === false){
      wordsWithoutPunct = wordsWithoutPunct + someString[k].toLowerCase();
    }
  }

  var newArray = wordsWithoutPunct.split(' ')
  for(var i = 0; i < newArray.length; i++){
    var makeProp = newArray[i];
    if(countObj[makeProp] === undefined){
      countObj[makeProp] = 1;
    }else{
...
var removePunctuation = function(word){
  var hasPuctuation = false;
  for(var j = 0; j < word.length; j++){
    if(word[j] === '.' || word[j] === ',' || word[j] === '!' || word[j] === '?')
    hasPuctuation = true;
  }
  return hasPuctuation;
}
