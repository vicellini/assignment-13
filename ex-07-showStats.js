...
var showStats = function(someNum, someArray){
  var dataObj = {
  ...
  for(var i = 0; i < someArray.length; i++){
    if(someArray[i] === someNum){
      dataObj.count_inputEqualTo = dataObj.count_inputEqualTo + 1;
    }
    if(someArray[i] < someNum){
      dataObj.count_inputGreaterThan = dataObj.count_inputGreaterThan + 1;
    }
    if(someArray[i] > someNum){
      dataObj.count_inputLessThan = dataObj.count_inputLessThan + 1;
    }
  }
  return dataObj;
}
